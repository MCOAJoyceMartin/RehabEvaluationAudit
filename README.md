...
- Produces one report covering every discipline in the batch (patient header, scores, criteria, evidence, comments, recommendations, corrective actions, strengths, additional findings, timeline, goal lifecycle, frequency analysis, documentation similarity) plus the batch-level cross-discipline consistency findings — deliberately excluding internal AI reasoning/rubric plumbing. "Print Audit" builds it as a PDF, opens it in a new tab, and downloads it to the Downloads folder, all in one click. "Save to OneDrive" builds the same PDF and sends it automatically (no download, no email draft) via a small relay — see `docs/POWER_AUTOMATE_SETUP.md`.
...
src/
  ...
  utils/                           dates.ts (calendar-day timeliness logic), exportReport.ts (HTML report body),
                                    reportToPdf.ts (renders that HTML to a PDF client-side, via jsPDF/html2canvas),
                                    saveToOneDrive.ts (builds the PDF and POSTs it to the relay endpoint)
```
...
- This build has no server component and no network calls other than loading the app's own static assets, **unless you configure "Save to OneDrive"** (see `docs/POWER_AUTOMATE_SETUP.md`), which POSTs the report as a PDF (containing PHI — name, MRN, DOB, diagnoses) to a Google Apps Script relay you control, which emails it onward. Before enabling it, confirm with your compliance team that the destination (the relay's Google account, the receiving mailbox, and the OneDrive/Power Automate environment) is appropriate for PHI — see the security note at the bottom of that doc, since the relay URL and its shared secret are both visible in this app's public client-side bundle.
